...
arduino = serial.Serial('COM16', 9600, timeout=0.1)
class vJoy(object):
...
def test():
    ...
    while True:  
        ...
        dataX=dataSplitted[0]
        dataY=dataSplitted[1]
        dataPER=dataSplitted[2]
       
        dataX = (dataX)[2:] #2den itibaren.
        dataX = (int(dataX) - 537)*3  
        dataY = (int(dataY) - 989)*129
        if(dataY < 0):
            dataY = 0;
        dataY = int(dataY)*-1
        if(dataPER[0] == '1'):
          dataY = 1500
        setJoy(dataX,dataY,10)
